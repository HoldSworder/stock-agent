...
> **同时贯穿一条顶层维度：短线 / 中线双模式**——短线赚情绪的钱（轻仓快打），中线赚趋势与产业景气的钱（重仓持有主升浪，如 2026 年 4 月起的通信 / 半导体 ETF 行情）。两套方法论正交，共用同一套模块、两套参数与逻辑。
...
## 3. 顶层维度：短线 / 中线双模式（horizon）

这是本次融合的核心：引入 `horizon = 'short' | 'mid'` 周期视角，**贯穿选股 / 决策 / 盯盘 / 战法四大模块**。不是新增模块，而是给现有模块装上「周期挡位」——同一套链路，两套参数与逻辑。三条闭环（A/B/C）在两种 horizon 下各跑一遍。
...
- **M1 ETF 行业轮动引擎（赛道层，中线发现起点）**：复用 [backend/src/etf/](backend/src/etf) 现成指标（双动量 R20/R60/R120、年线偏离、估值分位），新增行业景气度（[research](backend/src/research) 盈利预期）+ 趋势过滤（站上年线/MA200）+ 资金流多维打分，输出强赛道 TopN ETF，双周/月度调仓（偏差超 8% 才换）。
- **M2 中线选股档（赛道→个股下钻）**：[backend/src/screener/](backend/src/screener) `engines.ts` 新增中线策略——月线多头排列、周线缩量回踩/金叉、放量突破平台、右侧确认；选股可限定在 M1 强赛道 ETF 成分股内选龙头。
- **M3 中线决策档**：[backend/src/decision/](backend/src/decision) 按 horizon 调分析师权重，中线重景气/趋势/基本面、弱化游资情绪，核心问题从「今天买不买」变为「主升浪是否还在、该不该减」。
- **M4 中线盯盘档**：[backend/src/watch/](backend/src/watch) 中线只在跌破周线/月线、趋势破坏时告警，过滤日内噪声（贴合「拿得住」）。
- **M5 中线战法档**：[backend/src/strategy/](backend/src/strategy) profile 增加中线纪律——趋势不破不走、跌破 10 周线离场、核心卫星仓位、金字塔加减仓。

---

## 4. 目标架构：三条自动闭环 + 一个研判内核

> 下述三闭环在短线/中线两种 horizon 下各自成立：闭环 A 的「选股策略」「战法档案」「盯盘卖点」均分短线档与中线档；闭环 B 的「市场主线」天然偏中线赛道；闭环 C 的 ETF 正是中线主力载体。
...
- **短线门控**：情绪温度 / 涨跌家数 / 连板高度作为 screener 选股数量与 watch 告警阈值的门控（情绪退潮/冰点收紧，高潮减仓）。
- **中线门控**：行业景气周期 + 宏观状态（美林时钟）作为赛道轮动与中线建仓的门控（景气向上才进攻，景气退坡转防御/红利）。

让大盘从孤立看板变成「短线情绪 + 中线景气」双层全局开关。

---

## 5. 跨切面增量（横跨三闭环，真正帮到使用者）
...
双模式必须有清晰的全局切换入口：在选股 / 决策 / 盯盘 / 战法页顶部统一放「短线 ⇄ 中线」挡位开关，切换后该页的策略、指标口径、告警阈值、持有纪律随之切换。避免用户混淆「我现在看的是哪个周期的结论」。挡位状态可全局记忆（如存入通知/偏好 store），让系统记住你当前主打的周期。
...
| 阶段 M0 双模式基建 | `horizon=short\|mid` 贯穿 shared DTO 与四模块 + 周期切换 UI | 与阶段 0 一同打底 |
| 阶段 M1 ETF 行业轮动 | 赛道层景气+趋势+资金流打分、双周/月度调仓（复用 etf 指标） | 阶段 M0 |
| 阶段 M2 中线选股+下钻 | 月线/周线趋势因子 + 赛道内选龙头 | 阶段 M1 |
| 阶段 M3 中线决策/盯盘/战法档 | 四模块中线参数与逻辑 | 阶段 M0 |
| 阶段 M4 中线验证闭环 | 闭环 A 中线版 + 中线绩效/Alpha 报告 | 阶段 A、M1-M3 |
| 阶段 U UI 引导层 | 驾驶舱首页 + 标的时间线 + 渐进披露 + 一键动作 + 自动闭环可控 + 白话化 + 徽标提醒 + 周期切换 | 贯穿全程，随对应逻辑阶段配套交付 |

### 建议优先级

两条主线并行推进，按你的真实需求（更看重中线主升浪）中线线可前置：

- **短线/闭环线**：`阶段 0` → `阶段 A + D`（验证闭环及安全前提）→ `阶段 E` → `阶段 B/C` → `阶段 F/G`。
- **中线线**：`阶段 M0`（双模式基建，与阶段 0 一起打底）→ `阶段 M1`（ETF 行业轮动，直接服务你的 ETF 玩法、复用现成指标、最快见效）→ `阶段 M2`（赛道下钻选龙头）→ `阶段 M3/M4`。

两线共用基建：阶段 0 的复盘合并/去双跑、阶段 D 的账户总风控、阶段 C 的决策结论共享、阶段 E 的交易记忆，对短线与中线同时生效。UI 引导层（含 U9 周期切换）随对应逻辑阶段配套交付。
