#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ETF 短周期主线冲刺模式研究：不用 RS，用相邻短动量窗口寻找更早主线。

目标：
- 最大持仓 3 只；
- 入场只使用标的自身短周期绝对动量和趋势过滤，退出使用弱势替换和趋势破位；
- 用同一 ETF 池、同一 K 线数据源，与既有模式公平比较。

运行：
  python3 mode/etf-mainline-short-sprint/etf-mainline-short-sprint-research.py
...
OUT_DIR = os.environ.get(
    'SA_MODE_OUT_DIR',
    '/Users/qiuzhuoran/Desktop/自用/code/stock-agent/mode/etf-mainline-short-sprint/backtest-data',
)
TRADE_LOG = os.environ.get('SA_RESEARCH_TRADE_LOG', f'{OUT_DIR}/trades_short_sprint.md')
SUMMARY_LOG = os.environ.get('SA_RESEARCH_SUMMARY_LOG', f'{OUT_DIR}/summary_short_sprint.md')
...
cfgs = [
    ...
    for protect_gain, protect_dd, trend_ma, trend_dd, weak_rank in (
        (0.25, 0.15, 'ma60', 0.10, 6),
        (0.30, 0.18, 'ma60', 0.12, 9),
        (0.40, 0.22, 'ma120', 0.15, 12),
    )
]

tests = []
SCORE_KEYS = (
    'mom20', 'mom25', 'mom30', 'mom35', 'mom40', 'mom45', 'mom50', 'mom55', 'mom60',
...
for score_key in SCORE_KEYS:
    for abs_key in (None, 'above60', 'above120'):
        for rebalance in (1, 3, 5, 10):
            for cfg in cfgs:
                label = (
                    f'{score_key}|{abs_key or "noabs"}|{rebalance}d|pg{cfg["protect_gain"]:.0%}'
            ...
            for weights in ((1.0,), (0.95, 0.05), (0.9, 0.1), (0.85, 0.15), (0.8, 0.2), (0.7, 0.3), (0.6, 0.4), (0.5, 0.5), (0.5, 0.3, 0.2), (0.6, 0.25, 0.15), (0.45, 0.35, 0.20), (0.7, 0.2, 0.1)):
                for exit_rule in ('rank', 'ma120', 'ma60trail12'):
                    for reset_weights in (False, True):
                        ...
                        label = f'wrot-{mode}|{score_key}|{abs_key or "noabs"}|{rebalance}d|{"/".join(f"{w:.0%}" for w in weights)}|{exit_rule}'
                        tests.append(run_weighted_rotation(label, score_key, abs_key, rebalance, 0.0, weights, True, exit_rule=exit_rule, reset_weights=reset_weights))
...
def robust_candidate_key(r):
    return (
        round(r['ret'], 1),
        1 if not r.get('reset_weights', False) else 0,
        1 if r.get('abs') == 'above120' else 0,
        1 if r.get('exit_rule') == 'ma120' else 0,
        -r['trades'],
        r['mdd'],
    )

passed_ranked = sorted(passed, key=robust_candidate_key, reverse=True)
passed_high_with_exit = [r for r in passed_ranked if r['ret'] > HIGH_BAR_RET and r.get('exit_rule') in ('ma120', 'ma60trail12')]
passed_escape = [r for r in passed_ranked if r.get('exit_rule') == 'ma60trail12']
...
def rerun_like(r, label, start=None, end=None, log=False, px='close', cost_bps=0.0):
    if r['kind'] == 'weighted':
        return run_weighted_rotation(
            label, r['score'], r['abs'], r['rebalance'], 0.0, r['weights'], True,
            start, end, log, px, cost_bps, r['exit_rule'], r['reset_weights'],
        )
    return run_runner(
        label, r['score'], r['abs'], r['rebalance'], 0.0, True,
        start, end, log, px, cost_bps, r['cfg'],
    )

print(f'载入 {len(U)}/{len(POOL)} | 区间 {days[0]}→{days[-1]} | 策略族=短周期主线冲刺 | 基准收益>{BASELINE_RET:.1f}%')
print('Top30:')
...
with open(SUMMARY_LOG, 'w') as f:
    f.write('# ETF 短周期主线冲刺模式回测摘要\n\n')
    f.write(f'- 回测区间: {days[0]} 至 {days[-1]}\n')
    f.write(f'- ETF池: {len(U)}/{len(POOL)} 只可用\n')
    f.write(f'- 对比基准: 上一版 mom30 早期突破主线约 +{BASELINE_RET:.1f}%\n')
    f.write(f'- 最大持仓: 3；同主题最多 1 只；本轮搜索 1/3/5/10 个交易日调仓，候选为每 {best["rebalance"]} 个交易日调仓。\n')
    if best['kind'] == 'weighted':
        f.write(f'- 退出: 跌出加权 TopN；候选退出口径为 {best["exit_rule"]}；绝对趋势过滤为 {best["abs"] or "无"}；不按单标的定制。\n\n')
    else:
    ...
    f.write('\n## 收益集中度\n\n')
    f.write('按主题聚合，避免把同一主题的不同 ETF 代码误判为两个独立收益来源。\n\n')
...
print('收益贡献Top8:')
for fam, v in top_contrib[:8]:
    winrate = v['wins'] / v['count'] * 100 if v['count'] else 0
    names = '、'.join(f'{code} {name}' for code, name in sorted(v['codes'].items()))
    print(f'  {fam:<10} {names} 贡献{v["contrib"] * 100:+.1f}% / {v["count"]}笔 / 胜率{winrate:.0f}%')
