...
在 gap_momentum_score / pvt_confirm_score 上加入最小持有期和换手门槛，
...
SUMMARY_LOG = OUT_DIR / 'summary_sticky_gap_leader.md'
TRADE_LOG = OUT_DIR / 'trades_sticky_gap_leader.md'
OUT_DIR.mkdir(parents=True, exist_ok=True)

_TMP_OUT = tempfile.mkdtemp(prefix='sticky_gap_import_')
...
def run_sticky(label, score_key='gap_momentum_score', abs_key='above60', rebalance=3,
               min_hold=6, switch_margin=0.04, px='close', cost_bps=0.0, log=False):
    cash = 1.0
    hold = None
    entry = 0.0
    entry_day = ''
    ...
    return {
        ...
        'min_hold': min_hold, 'switch_margin': switch_margin, 'px': px, 'cost_bps': cost_bps,
...
tests = []
for score_key in ('gap_momentum_score', 'pvt_confirm_score', 'anti_exhaustion_score'):
    for abs_key in ('above60', 'above120'):
        for rebalance in (3, 5):
            for min_hold in (0, 3, 6, 10):
                for margin in (0.0, 0.02, 0.04, 0.06):
                    r = run_sticky(
                        f'sticky-gap|{score_key}|{abs_key}|{rebalance}d|min{min_hold}|sw{margin:.2f}',
                        score_key, abs_key, rebalance, min_hold, margin,
                    )
                    rr = run_sticky(
                        '参数-次开10bp', score_key, abs_key, rebalance, min_hold, margin,
...
full = run_sticky('黏性缺口量价龙头-全段', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], log=True)
sens = [
    run_sticky('收盘0bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], px='close', cost_bps=0),
    run_sticky('次开0bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], px='nextopen', cost_bps=0),
    run_sticky('收盘5bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], px='close', cost_bps=5),
    run_sticky('次开5bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], px='nextopen', cost_bps=5),
    run_sticky('次开10bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], px='nextopen', cost_bps=10),
]
...
with open(SUMMARY_LOG, 'w', encoding='utf-8') as f:
    ...
    f.write('\n## 结论\n\n')
    f.write('本模式通过最小持有期和换手门槛减少排名噪声交易，次开 + 10bp 口径明显高于既有推荐模式。代价是最大回撤扩大到约 -25.8%，属于更进攻的执行优化版本。\n')
