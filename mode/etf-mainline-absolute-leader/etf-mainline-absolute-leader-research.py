#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ETF 绝对动量龙头模式研究：不用 RS，只用标的自身趋势选主线。

目标：
- 最大持仓 3 只；
- 入场只使用标的自身绝对动量/多周期趋势，退出使用弱势替换和趋势破位；
- 用同一 ETF 池、同一 K 线数据源，与既有模式公平比较。

运行：
  python3 mode/etf-mainline-absolute-leader/etf-mainline-absolute-leader-research.py
...
OUT_DIR = os.environ.get(
    'SA_MODE_OUT_DIR',
    '/Users/qiuzhuoran/Desktop/自用/code/stock-agent/mode/etf-mainline-absolute-leader/backtest-data',
)
TRADE_LOG = os.environ.get('SA_RESEARCH_TRADE_LOG', f'{OUT_DIR}/trades_absolute_leader.md')
SUMMARY_LOG = os.environ.get('SA_RESEARCH_SUMMARY_LOG', f'{OUT_DIR}/summary_absolute_leader.md')
...
tests = []
SCORE_KEYS = ('mom60', 'mom90', 'mom120', 'abs13612', 'abs_mom_blend', 'abs_accel')
...
for score_key in SCORE_KEYS:
    for abs_key in (None, 'above60', 'above120'):
        for weights in ((1.0,), (0.85, 0.15), (0.8, 0.2), (0.7, 0.3), (0.6, 0.4), (0.5, 0.5), (0.5, 0.3, 0.2), (0.6, 0.25, 0.15), (0.45, 0.35, 0.20), (0.7, 0.2, 0.1)):
...
print(f'载入 {len(U)}/{len(POOL)} | 区间 {days[0]}→{days[-1]} | 策略族=绝对动量龙头 | 基准收益>{BASELINE_RET:.0f}% | 最高参考>{HIGH_BAR_RET:.0f}%')
print('Top30:')
...
with open(SUMMARY_LOG, 'w') as f:
    f.write('# ETF 绝对动量龙头模式回测摘要\n\n')
    f.write(f'- 回测区间: {days[0]} 至 {days[-1]}\n')
    f.write(f'- ETF池: {len(U)}/{len(POOL)} 只可用\n')
    f.write(f'- 对比基准: 上一版推荐 Top2 逃顶约 +{BASELINE_RET:.0f}%；上一版最高收益约 +{HIGH_BAR_RET:.1f}%\n')
    ...
    f.write('## Top10 参数\n\n')
    f.write('| 策略 | 收益 | 年化 | 最大回撤 | 交易 | 均仓 | 空槽日 | 超推荐版 | 超最高版 |\n')
    f.write('|---|---:|---:|---:|---:|---:|---:|---|---|\n')
    for r in tests[:10]:
        f.write(f'| {mdcell(r["label"])} | {r["ret"]:.1f}% | {r["ann"]:.1f}% | {r["mdd"]:.1f}% | {r["trades"]} | {r["avgheld"]:.2f} | {r["underinvested_days"]} | {"是" if r["ret"] > BASELINE_RET else "否"} | {"是" if r["ret"] > HIGH_BAR_RET else "否"} |\n')
    f.write('\n## 候选复核\n\n')
