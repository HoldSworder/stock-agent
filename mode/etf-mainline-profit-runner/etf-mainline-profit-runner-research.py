#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ETF 主线利润奔跑模式研究：主线强度入场 + 盈利腿趋势持有。

目标：
- 最大持仓 3 只；
- 入场只使用主线/相对强度，退出使用弱势替换、趋势破位和盈利保护；
- 用同一 ETF 池、同一 K 线数据源，与既有模式公平比较。

运行：
  python3 mode/etf-mainline-profit-runner/etf-mainline-profit-runner-research.py
...
OUT_DIR = os.environ.get(
    'SA_MODE_OUT_DIR',
    '/Users/qiuzhuoran/Desktop/自用/code/stock-agent/mode/etf-mainline-profit-runner/backtest-data',
)
TRADE_LOG = os.environ.get('SA_RESEARCH_TRADE_LOG', f'{OUT_DIR}/trades_profit_runner.md')
SUMMARY_LOG = os.environ.get('SA_RESEARCH_SUMMARY_LOG', f'{OUT_DIR}/summary_profit_runner.md')
...
for code, name in POOL.items():
    ...
    for i, b in enumerate(bars):
        ...
        by[d] = {
            ...
            'combo_rs_mom': rsret(90) + 0.5 * ret(90),
            'above60': b['c'] > ma60[i],
...
def run_runner(label, score_key='rs90', abs_key=None, rebalance=5, threshold=0.0,
...
def run_weighted_rotation(label, score_key='rs90', abs_key=None, rebalance=5, threshold=0.0,
...
tests = []
SCORE_KEYS = ('rs60', 'rs90', 'rs120', 'mom90', 'combo90', 'combo60_90', 'combo_rs_mom')
...
for score_key in SCORE_KEYS:
    for abs_key in (None, 'above60', 'above120'):
        for weights in ((1.0,), (0.7, 0.3), (0.6, 0.4), (0.5, 0.5), (0.5, 0.3, 0.2), (0.6, 0.25, 0.15), (0.45, 0.35, 0.20), (0.7, 0.2, 0.1)):
...
best = passed_escape[0] if passed_escape else (passed[0] if passed else tests[0])
...
print(f'载入 {len(U)}/{len(POOL)} | 区间 {days[0]}→{days[-1]} | 策略族=主线利润奔跑 | 基准收益>{BASELINE_RET:.0f}%')
print('Top30:')
