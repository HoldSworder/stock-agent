...
在黏性 anti_exhaustion_score 龙头模式上加入持仓内盈利保护：
单笔浮盈超过 25% 后，如从持仓峰值回撤 8%，优先锁定主升收益。
"""
import importlib
import math
import os
...
SUMMARY_LOG = OUT_DIR / 'summary_profit_protected_sticky_leader.md'
TRADE_LOG = OUT_DIR / 'trades_profit_protected_sticky_leader.md'
OUT_DIR.mkdir(parents=True, exist_ok=True)

_TMP_OUT = tempfile.mkdtemp(prefix='profit_protected_sticky_import_')
...
def run_sticky(label, score_key='anti_exhaustion_score', abs_key='above60', rebalance=3,
    ...
    return {
        ...
        'protect_gain': protect_gain, 'protect_dd': protect_dd, 'px': px, 'cost_bps': cost_bps,
...
tests = []
for protect_gain in (0.20, 0.25, 0.30):
    for protect_dd in (0.06, 0.08, 0.10):
        for min_hold in (4, 6, 8):
            for margin in (0.035, 0.04, 0.05):
                r = run_sticky(
                    f'profit-protected-sticky|anti_exhaustion_score|above60|3d|min{min_hold}|sw{margin:.3f}|top2|pg{protect_gain:.0%}|dd{protect_dd:.0%}',
                    'anti_exhaustion_score', 'above60', 3, min_hold, margin, 2, protect_gain, protect_dd,
                )
                rr = run_sticky(
                    '参数-次开10bp', 'anti_exhaustion_score', 'above60', 3, min_hold, margin, 2,
                    protect_gain, protect_dd, px='nextopen', cost_bps=10,
                )
                tests.append((rr['ret'], r, rr))
tests.sort(key=lambda x: x[2]['flat_ret'], reverse=True)
best_next, best, best_next_run = tests[0]
full = run_sticky(
    '盈利保护黏性主线龙头-全段', best['score'], best['abs'], best['rebalance'], best['min_hold'],
    best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], log=True,
)
sens = [
    run_sticky('收盘0bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], px='close', cost_bps=0),
    run_sticky('次开0bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], px='nextopen', cost_bps=0),
    run_sticky('收盘5bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], px='close', cost_bps=5),
    run_sticky('次开5bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], px='nextopen', cost_bps=5),
    run_sticky('次开10bp', best['score'], best['abs'], best['rebalance'], best['min_hold'], best['switch_margin'], best['top_exit'], best['protect_gain'], best['protect_dd'], px='nextopen', cost_bps=10),
]
records = write_records(TRADE_LOG, full['records'])

baseline_close = 599.2
baseline_next10 = 529.8
...
with open(SUMMARY_LOG, 'w', encoding='utf-8') as f:
    f.write('# 盈利保护黏性主线龙头回测摘要\n\n')
    f.write(f'- 回测区间: {days[0]} 至 {days[-1]}\n')
    f.write(f'- ETF池: {len(U)} 只\n')
    f.write('- 最大持仓: 1（满足最大持仓不超过 3）；同主题重复持仓天然为 0。\n')
    f.write(f'- 推荐配置: `{best["label"]}`\n')
    f.write(f'- 机制: 最小持有 `{best["min_hold"]}` 个交易日；新标的分数至少高出 `{best["switch_margin"]:.3f}` 才触发强者替换；持仓跌出 Top{best["top_exit"]}、跌破 MA120、或单笔浮盈超过 `{best["protect_gain"]:.0%}` 后从峰值回撤 `{best["protect_dd"]:.0%}` 时退出。\n')
    ...
    f.write(f'- 基准: 黏性反枯竭龙头收盘 `{baseline_close:.1f}%`；黏性反枯竭龙头次开10bp `{baseline_next10:.1f}%`；本模式次开10bp `{sens[-1]["ret"]:.1f}%`。\n\n')
    f.write('## 候选复核\n\n')
    ...
    f.write('\n## 结论\n\n')
    f.write('本模式在不增加持仓数量、不引入文本未来信息的前提下，把盈利保护作为持仓内退出层：主升段继续让利润奔跑，主升后明显回撤则锁定收益。相对原黏性反枯竭龙头，收益显著提高；收盘口径最大回撤持平，次开成本口径回撤略低。它仍属于单腿集中进攻模式，需要继续前向验证。\n')
