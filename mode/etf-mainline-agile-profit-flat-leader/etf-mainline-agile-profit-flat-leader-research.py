...
SUMMARY_LOG = OUT_DIR / 'summary_agile_profit_flat_leader.md'
TRADE_LOG = OUT_DIR / 'trades_agile_profit_flat_leader.md'
OUT_DIR.mkdir(parents=True, exist_ok=True)

_TMP_OUT = tempfile.mkdtemp(prefix='agile_profit_flat_import_')
...
def load_research_prefix():
    ns = {'__file__': str(SWEEP), '__name__': 'agile_profit_flat_prefix'}
...
def candidates(date, confirm_min=0.15):
    ...
    for code, (_, by) in U.items():
        ...
        if valid(persist) and persist >= confirm_min and valid(score):
...
def run_mode(label, rebalance=4, min_hold=6, switch_margin=0.0, top_exit=2,
             protect_gain=0.15, protect_dd=0.06, confirm_min=0.15,
...
RECOMMENDED = {
    ...
    'protect_gain': 0.15, 'protect_dd': 0.06, 'confirm_min': 0.15, 'exit_ma': 'ma120',
}
...
for rebalance in (3, 4, 5):
    for min_hold in (4, 6, 8):
        for switch_margin in (0.0, 0.01, 0.02, 0.035):
            for top_exit in (1, 2, 3, 4):
                for protect_gain, protect_dd in ((0.15, 0.06), (0.20, 0.08), (0.25, 0.08), (0.30, 0.10)):
                    for confirm_min in (0.12, 0.15, 0.18):
                        label = (
                            f'agile-profit-flat|anti_exhaustion_score|above60|{rebalance}d|'
                            f'min{min_hold}|sw{switch_margin:g}|top{top_exit}|'
                            f'pg{protect_gain:.0%}|dd{protect_dd:.0%}|mainline_persist>={confirm_min:.2f}'
                        )
                        close = run_mode(label, rebalance, min_hold, switch_margin, top_exit, protect_gain, protect_dd, confirm_min, 'ma120')
                        nxt = run_mode(label, rebalance, min_hold, switch_margin, top_exit, protect_gain, protect_dd, confirm_min, 'ma120', px='nextopen', cost_bps=10)
                        tests.append((close['flat_ret'], nxt['flat_ret'], close, nxt))
tests.sort(key=lambda x: (x[0], x[1]), reverse=True)

full = run_mode('敏捷止盈非复利主线龙头-全段', **RECOMMENDED, log=True)
...
with open(SUMMARY_LOG, 'w', encoding='utf-8') as f:
    f.write('# 敏捷止盈非复利主线龙头回测摘要\n\n')
    f.write(f'- 回测区间: {days[0]} 至 {days[-1]}\n')
    f.write(f'- ETF池: {len(U)} 只\n')
    f.write('- 最大持仓: 1（满足最大持仓不超过 3）；同主题重复持仓天然为 0。\n')
    f.write('- 推荐配置: `agile-profit-flat|anti_exhaustion_score|above60|4d|min6|sw0|top2|pg15%|dd6%|mainline_persist>=0.15`\n')
    f.write('- 机制: 候选需 `mainline_persist >= 0.15`；每 4 个交易日复核；最小持有 6 个交易日；更强候选出现即可替换；持仓跌出 Top2、跌破 MA120、或单笔浮盈超过 15% 后从峰值回撤 6% 时退出。\n')
    ...
    f.write('- 说明: 本模式按系统展示的收盘口径 `flatReturn` 优先，收盘非复利高于 `高持续主线非复利龙头` 的 `236.5%`；但次开10bp非复利略低于其 `234.6%`，因此实盘执行需额外观察滑点。\n\n')
    ...
    f.write('|---|---:|---:|---:|---:|---:|---:|---|\n')
    rec_label = 'agile-profit-flat|anti_exhaustion_score|above60|4d|min6|sw0|top2|pg15%|dd6%|mainline_persist>=0.15'
    ...
    f.write('\n## 结论\n\n')
    f.write('本模式是收盘口径 flatReturn 优先的进攻版本。它通过更敏捷的盈利保护提高非复利主指标，但次开10bp口径没有同步创新高，因此更适合继续观察而不是替代所有执行场景。\n')
