...
在“主线确认盈利保护黏性龙头”基础上，把搜索目标切到非复利收益：
提高 mainline_persist 门槛，只追持续性更强的主线，同时降低切换阈值，
让持仓更快跟随已经确认的主线龙头。
"""
import math
import os
...
SUMMARY_LOG = OUT_DIR / 'summary_high_persist_flat_leader.md'
TRADE_LOG = OUT_DIR / 'trades_high_persist_flat_leader.md'
OUT_DIR.mkdir(parents=True, exist_ok=True)

_TMP_OUT = tempfile.mkdtemp(prefix='high_persist_flat_import_')
...
def load_research_prefix():
    """加载 factor sweep 的指标构造部分，跳过默认大扫，保证本脚本可快速复跑。"""
    ns = {'__file__': str(SWEEP), '__name__': 'high_persist_flat_prefix'}
...
def candidates(date, confirm_min=0.15):
    rows = []
    for code, (_, by) in U.items():
        ...
        if not (valid(persist) and persist >= confirm_min):
            continue
...
def run_mode(label, rebalance=3, min_hold=4, switch_margin=0.01, top_exit=2,
             protect_gain=0.25, protect_dd=0.08, confirm_min=0.15,
    ...
    for idx, date in enumerate(active_days):
        ...
        if idx % rebalance != 0:
            continue
        ranked = candidates(date, confirm_min)
        if not ranked:
            continue
...
def rerun(label, **kwargs):
    return run_mode(label, **kwargs)


RECOMMENDED = {
    'rebalance': 3, 'min_hold': 4, 'switch_margin': 0.01, 'top_exit': 2,
    'protect_gain': 0.25, 'protect_dd': 0.08, 'confirm_min': 0.15, 'exit_ma': 'ma120',
}

tests = []
for confirm in (0.10, 0.12, 0.15, 0.18, 0.20):
    for rebalance in (2, 3, 4, 5):
        for min_hold in (3, 4, 6, 8):
            for switch_margin in (0.0, 0.01, 0.02, 0.035):
                for protect_gain, protect_dd in ((0.15, 0.06), (0.20, 0.08), (0.25, 0.08), (0.30, 0.10), (0.40, 0.12)):
                    label = (
                        f'high-persist-flat|anti_exhaustion_score|above60|{rebalance}d|'
                        f'min{min_hold}|sw{switch_margin:g}|top2|pg{protect_gain:.0%}|'
                        f'dd{protect_dd:.0%}|mainline_persist>={confirm:.2f}'
                    )
                    close = run_mode(label, rebalance, min_hold, switch_margin, 2, protect_gain, protect_dd, confirm, 'ma120')
                    nxt = run_mode(label, rebalance, min_hold, switch_margin, 2, protect_gain, protect_dd, confirm, 'ma120', px='nextopen', cost_bps=10)
                    tests.append((nxt['flat_ret'], close['flat_ret'], close, nxt))
...
with open(SUMMARY_LOG, 'w', encoding='utf-8') as f:
    f.write('# 高持续主线非复利龙头回测摘要\n\n')
    f.write(f'- 回测区间: {days[0]} 至 {days[-1]}\n')
    f.write(f'- ETF池: {len(U)} 只\n')
    f.write('- 最大持仓: 1（满足最大持仓不超过 3）；同主题重复持仓天然为 0。\n')
    f.write('- 推荐配置: `high-persist-flat|anti_exhaustion_score|above60|3d|min4|sw0.01|top2|pg25%|dd8%|mainline_persist>=0.15`\n')
    f.write('- 机制: 候选需 `mainline_persist >= 0.15`；每 3 个交易日复核；最小持有 4 个交易日；新标的分数至少高出 0.01 才触发强者替换；持仓跌出 Top2、跌破 MA120、或单笔浮盈超过 25% 后从峰值回撤 8% 时退出。\n')
    ...
    f.write('- 说明: 本轮排序以次开10bp非复利收益为准。该模式高于上一版主线确认盈利保护黏性龙头的收盘非复利 `223.1%` 与次开10bp非复利 `214.8%`。\n\n')
    ...
    f.write('## 参数对照（按次开10bp非复利收益排序）\n\n')
    f.write('| 策略 | 收盘复利 | 收盘非复利 | 最大回撤 | 交易 | 次开10bp收益 | 次开10bp非复利 | 备注 |\n')
    f.write('|---|---:|---:|---:|---:|---:|---:|---|\n')
    for _, _, close, nxt in tests[:24]:
        note = '推荐' if close['label'] == 'high-persist-flat|anti_exhaustion_score|above60|3d|min4|sw0.01|top2|pg25%|dd8%|mainline_persist>=0.15' else ''
    ...
    f.write('\n## 结论\n\n')
    f.write('本模式在非复利收益口径上超过上一版最佳模式。优势来自更严格的主线持续性确认和更敏捷的强者替换；邻近参数中 `sw0.01~0.02`、`min4~6`、`pg20%~25%/dd8%` 均高于现有非复利基准。风险是 `mainline_persist >= 0.15` 会推迟早期主线捕捉，因此应继续前向观察。\n')
