...
export async function buildHotConcepts(window: ConceptWindow = '5日'): Promise<HotConceptOverview> {
  ...
  return {
    ...
    flat,
    note:
      `热门细分概念（同花顺概念资金流·近${window}，按规则计算，仅供参考，不构成投资建议）：按 近${window}涨幅 + 资金净额 两维归一加权合成热度分，` +
      '并按关键词归纳到父级主线主题，便于「锁强赛道 → 从赛道里挑龙头」。点击概念可展开板块全部成分股（经问财，标注龙头/今日领涨）。' +
      (stale
        ? '⚠️ 同花顺概念资金流没取到，榜为不完整估计（请到数据源页检查 AKShare/aktools 配置）。'
        : ''),
    stale,
  };
}
