...
import { buildMoneyEffectOverview, formatMoneyEffectForAgent } from '../moneyeffect/service';
...
export async function buildMarketBoardPrompt(ov: MarketOverview): Promise<string> {
  ...
  // 首板赚钱效应确定性底稿（883994·打板追涨情绪温度计）：best-effort，失败则跳过该段。
  let moneyEffectBlock = '';
  try {
    const me = await buildMoneyEffectOverview(false);
    moneyEffectBlock = `\n\n【首板赚钱效应·确定性底稿】\n${formatMoneyEffectForAgent(me)}\n`;
  } catch {
    /* 883994 取数失败：研判退回不含首板赚钱效应底稿 */
  }
