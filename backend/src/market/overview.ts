...
export async function buildMarketBoardPrompt(ov: MarketOverview): Promise<string> {
  ...
  return (
    ...
    '第3步 期货+外盘传导（据快照 futures 国内外商品期货 + globalIndices 隔夜外盘）：①梳理有色/黑色/贵金属/能化/新能源等商品涨跌逻辑，判断对 A 股相关产业链板块的传导方向（受益/承压）；②结合隔夜外盘，重点参考中概金龙指数（最直接映射 A 股情绪）与 VIX 恐慌指数（避险情绪），预判次日开盘情绪与方向。\n' +
