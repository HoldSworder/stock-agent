...
<template>
  <el-drawer v-model="visible" title="强势方法论 · 指标怎么算" size="440px" direction="rtl">
    <div class="mt">
      <el-segmented
        v-model="active"
        :options="[
      ...
      <section v-show="active === 'short'" class="sec">
        ...
        <p class="tip">综合分点开可见各因子「score×权重=贡献」；每个策略的筛选条件见结果头「硬性条件」。</p>
      </section>
      ...
      <section v-show="active === 'mid'" class="sec">
        ...
        <ul>
          ...
          <li><b>多日算法</b>60日涨幅(f24)为<b>板块级真实</b>多日强弱；20日动能 / 趋势 / 年线偏离为<b>龙头个股代理</b>（板块日 K 不可得时以领涨/龙头个股 K 线近似）。</li>
          ...
          <li><b>agent 过滤</b>「板块主线研判」读这份按规则算出的原始数据，过滤出可信主线 / 值得跟踪行业 / 应剔除噪声，结论置于 Tab 顶部并进今日计划。</li>
        </ul>
      ...
      <section v-show="active === 'rotation'" class="sec">
        ...
        <ul>
          ...
          <li><b>agent 过滤</b>「ETF行业轮动研判」读这份按规则算出的轮动榜，过滤出该进攻 / 该等回踩 / 该回避赛道，结论置于 Tab 顶部并进今日计划第六源。</li>
        </ul>
        <p class="tip">纪律：涨幅靠后≠该卖（看趋势与 RS）；过热≠还能涨（应等回踩而非追高）；不读研报景气，只信量价与资金的实际读数。</p>
      </section>
      ...
      <section class="guide">
        ...
        <p class="note">强弱均由代码按固定规则算出（均线 / 分位 / 理想点 / 动量 / 资金），可复现可审计；中线主线再经「板块主线研判」agent 过滤后进今日计划。</p>
      </section>
    </div>
  </el-drawer>
</template>
