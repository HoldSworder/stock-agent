...
| R13 | 采纳削减（后续按需求放开至展示层） | 原：波浪首期仅允许出现在解释文本，不进DTO、图层、状态机和验收门禁。现已放开确定性计数进入DTO与K线图层（详见 4.5），但仍不进候选目录、状态机与回测 |
...
波浪理论原定首期不进入结构化DTO、状态机、结构化情景、图层或验收门禁。

**该约束已按需求部分放开，放开范围仅限展示层**：`backend/src/market/elliott.ts` 提供确定性波浪计数（艾略特三铁律 + 斐波那契目标投射 + bar数时间窗外推），经 `GET /api/stock/:code/elliott` 供 `KlineDialog` 的「波浪」图层与结论区展示，DTO 为 `ElliottAnalysis`。

放开之外的边界一律不变，实现与评审时按此对照：

- 不进入候选价位/候选条件目录（`symbolPlans/candidateCatalog.ts`），不参与 `SymbolTradePlan` 的触发、失效、目标判定；
- 不进入统一市场阶段状态机，不影响阶段判定与默认应对；
- 不纳入规则回测与胜率统计（口径见 4.10）；
- LLM 解读（`POST /api/stock/:code/elliott/interpret`）只能复述后端已算出的浪序与点位，不得改浪、不得出现“根据波浪所以买入/卖出”。
